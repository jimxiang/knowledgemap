        ...
        try:
            ...
            for i in range(len(_links)):
                ...
                if node_eid:
                    ...
                    queue.put(json.dumps({"relation": json.dumps(temp)}))
                    queue.put(json.dumps({"mapping": json.dumps({"union_id": union_id, "eid": node_eid})}))
        except (nx.NetworkXException, nx.NetworkXNoPath):
            return []
...
def process_small_graph(graph, queue):
    try:
        ...
        union_id = create_union_id()
        queue.put(json.dumps({"relation": json.dumps({"id": union_id, "value": value})}))

        for node in all_nodes:
            eid = NODE.get_node_eid(node)
            if eid:
                queue.put(json.dumps({"mapping": json.dumps({"union_id": union_id, "eid": eid})}))
...
def main(rel_path, eid_mapping_path, cutoff=10):
    ...
    writer = mp.Process(target=customer, args=(queue, lock, f_rel, f_eid,))
